...
*   **權重調整 (Weight Adjustment)**：
    *   每個 Chip 上都有 **`−` `+`** 按鈕，平時淡化，滑鼠移到 Chip 上會變明顯。
    *   也可以停留在 Chip 上按住 **Shift + 滾動滑鼠滾輪**。
    *   權重不等於 1.0 時，`−` `+` 中間會顯示數字；**點那個數字可重設回 1.0**。
    *   範圍 **0.1 ~ 2.0**，每次 0.1。
    *   Chip 上只顯示標籤本體。權重格式 `(text:1.1)` 是存檔與輸出用的，不拿來給人讀。
*   **拖曳重排 (Drag & Drop)**：
...
*   **不夠位置就往上開**：下拉選單在下方空間不足時會自動往上展開，不會被節點下緣切掉。

### 📐 節點高度

*   高度**完全由你拖曳決定**，可以縮到很小（下限與 ComfyUI 其他節點一致）。
*   內容超過高度時，整個編輯區出現一個捲軸。
*   建立節點或載入 workflow 時會**自動貼合內容一次**，之後不再自己長高 —— 你縮小它，它就維持縮小。
